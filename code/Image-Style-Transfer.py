...
# 输出图像的所需尺寸
imsize = 1024 if torch.cuda.is_available() else 256  # 如果没有GPU，请使用小尺寸
...
def image_loader(image_name):
    image = Image.open(image_name).convert('RGB')  # 确保图像是RGB模式
    image = loader(image).unsqueeze(0)  # 需要伪造的批次尺寸以适合网络的输入尺寸
    return image.to(device, torch.float)

# 加载样式和内容图像
style_img = image_loader(r"F:\Mouselnc\致美化\文件转移\Desktop\人工智能项目\代码文件\图片\风格图片\111.jpg")
content_img = image_loader(r"F:\Mouselnc\致美化\文件转移\Desktop\人工智能项目\代码文件\图片\内容图片\wxr.jpg")

# 调整图像大小
def resize_image_to_match(image, target_image):
    return transforms.functional.resize(image, target_image.size()[2:])

style_img = resize_image_to_match(style_img, content_img)
...
# 调整训练步数和损失函数权重
num_steps = 200  # 增加训练步数
style_weight = 1e6  # 增加风格权重
content_weight = 50  # 内容权重保持不变
